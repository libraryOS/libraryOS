...
<x-form id="new-office-form" x-target="office-list new-office-form notifications" x-target.back="new-office-form" action="{{ route('organization.adminland.office.store', $organization->slug) }}" method="post" class="space-y-5 rounded-t-lg border-b border-gray-200 p-4 hover:bg-blue-50 dark:border-gray-700 dark:hover:bg-gray-800">
  <div>
    <x-input id="name" :label="__('Name')" type="text" name="name" required autofocus :error="$errors->get('name')" value="{{ old('name') }}" />
  </div>

  <div>
    <x-select id="office_type_id" :label="__('Office type')" :options="$officeTypeOptions" :selected="old('office_type_id', '')" :error="$errors->get('office_type_id')" />
  </div>
  ...
  <div class="flex justify-between">
    <x-button.secondary href="{{ route('organization.adminland.office.index', $organization->slug) }}" turbo="true" x-target="new-office-form">
      {{ __('Cancel') }}
    </x-button.secondary>
