  ...
  <div class="grid grow bg-gray-50 sm:grid-cols-[220px_1fr] dark:bg-gray-950">
    ...
    <section class="p-4 sm:p-8">
      <div class="mx-auto max-w-5xl space-y-6 sm:px-0">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $patronType->name }}</h2>

        <form action="{{ route('organization.adminland.patron-type.update', [$organization->slug, $patronType->id]) }}" method="post" class="space-y-8">
          @csrf
          @method('PUT')
          ...
          <div class="grid gap-6 sm:grid-cols-3">
            ...
            <div class="rounded-lg border border-gray-200 bg-white p-6 sm:col-span-2 dark:border-gray-700 dark:bg-gray-900">
              <div class="grid gap-4 sm:grid-cols-2">
                <x-input id="name" :label="__('Name')" type="text" name="name" required autofocus :error="$errors->get('name')" value="{{ old('name', $patronType->name) }}" />
                <x-input id="key" :label="__('Key')" type="text" name="key" required :error="$errors->get('key')" value="{{ old('key', $patronType->key) }}" />
              </div>
            </div>
          </div>
              ...
              <div class="space-y-4">
                <div class="flex items-start gap-3">
                  <div class="flex h-6 items-center">
                    ...
                    <input id="is_active" name="is_active" type="checkbox" value="1" @checked(old('is_active', $patronType->is_active)) class="h-4 w-4 rounded border-gray-300 accent-emerald-600 dark:border-gray-600" />
                  </div>
                  <div class="text-sm">
                ...
                <div class="grid gap-4 pt-2 sm:grid-cols-2">
                  <x-input id="membership_duration_days" :label="__('Membership duration (days)')" type="number" name="membership_duration_days" :error="$errors->get('membership_duration_days')" value="{{ old('membership_duration_days', $patronType->membership_duration_days) }}" />
                  <x-input id="max_loans" :label="__('Max simultaneous loans')" type="number" name="max_loans" :error="$errors->get('max_loans')" value="{{ old('max_loans', $patronType->max_loans) }}" />
                  <x-input id="minimum_age" :label="__('Minimum age')" type="number" name="minimum_age" :error="$errors->get('minimum_age')" value="{{ old('minimum_age', $patronType->minimum_age) }}" />
                  <x-input id="maximum_age" :label="__('Maximum age')" type="number" name="maximum_age" :error="$errors->get('maximum_age')" value="{{ old('maximum_age', $patronType->maximum_age) }}" />
                </div>
              </div>
            </div>
          </div>
              ...
              <div class="space-y-4">
                <div class="flex items-start gap-3">
                  <div class="flex h-6 items-center">
                    ...
                    <input id="keep_loan_history" name="keep_loan_history" type="checkbox" value="1" @checked(old('keep_loan_history', $patronType->keep_loan_history)) class="h-4 w-4 rounded border-gray-300 accent-emerald-600 dark:border-gray-600" />
                  </div>
                  <div class="text-sm">
                ...
                <div class="flex items-start gap-3">
                  <div class="flex h-6 items-center">
                    ...
                    <input id="can_receive_notifications" name="can_receive_notifications" type="checkbox" value="1" @checked(old('can_receive_notifications', $patronType->can_receive_notifications)) class="h-4 w-4 rounded border-gray-300 accent-emerald-600 dark:border-gray-600" />
                  </div>
                  <div class="text-sm">
          ...
          <div class="grid gap-6 sm:grid-cols-3">
            ...
            <div class="rounded-lg border border-gray-200 bg-white p-6 sm:col-span-2 dark:border-gray-700 dark:bg-gray-900">
              <x-input id="description" :label="__('Description')" type="text" name="description" :error="$errors->get('description')" value="{{ old('description', $patronType->description) }}" />
            </div>
          </div>
