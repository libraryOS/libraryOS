...
let root = {
  'x-data'() {
    return {
      ...
      close(focus = true) {
        this.expanded = false;
        focus && !document.activeElement.isSameNode(this.trigger) && setTimeout(() => this.trigger.focus());
      },
      toggle() {
...
let trigger = {
  ...
  ':aria-controls'() {
    return this.$data.expanded && this.$id('popover_menu');
  },
  '@click'() {
    this.$data.toggle();
  },
...
let menu = {
  ...
  '@mousedown.window'($event) {
    this.$data.expanded && !this.$data.trigger.contains($event.target) && !this.$el.contains($event.target) && this.$data.close();
  },
};
